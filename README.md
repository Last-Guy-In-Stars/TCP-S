...
Клиент (с tcps.ko)                    Сервер (с tcps.ko)
═══════════════                        ═══════════════
1. SYN + TCP option "TC" ──────────►  2. Видит "TC" — понимает,
                                          что клиент поддерживает TCPS
                                    
3. ◄────────────── SYN-ACK + TCP option "TC"
   Видит "TC" — понимает,              4. Обе стороны знают:
      что сервер поддерживает              можно шифровать
5. ◄─────── Curve25519 pubkey (32b) ──►   Обмен публичными ключами
       Curve25519 pubkey (32b) ──────►     как первые байты данных
6. Обе стороны вычисляют shared secret → derive enc/dec ключи
7. ◄══════ ChaCha20 зашифрованный поток ══════►
   Приложения (postgres, nginx, curl) 
   ничего не знают — шифрование в ядре
...
# Всё. Любое TCP-подключение между этими машинами 
# автоматически шифруется. PostgreSQL, HTTP, SSH — что угодно.
