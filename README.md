...
10. GSO packets are processed: GSO is disabled on the socket (sk_gso_type=0); if the race window is low, software segmentation + encryption of each segment is used.

# Init-key exchange — key exchange protocol

Each node generates an init_key (random 32B) upon boot. **pub_key** = curve25519(init_key, base).
...
- Tag is truncated to 4 bytest (2^32 forgery resistance)
...
**Behavior:**
- TM option is NOT removed on reception — TCP stack ignores unknown kind=253
- GSO packets: `sk_gso_type=0` disables GSO on the socket; Fallback — `skb_gso_segment()` + encryption of each segment + `ip_local_out()` with mark for loop protection
- RST without MAC in the KEYED state → DROP (injection protection)
...
- The recipient verifies the auth_tag with prev_psk (constant-time comparison)
- If tag matches → KEYXCHG_AUTH verified (rotation is legitimate)
- If tag does not match → WARN, continue as plain KEYXCHG (prev_psk is out of sync during simultaneous rotation)
...
4. The old init_key is saved as prev_init_key (for DH retry when decrypting KEYXCHG)
...
**Problem:** A MITM attack can replace public keys in a UDP exchange → Both nodes receive different PSKs → The MITM attack can decrypt both sides.
...
**Problem:** GSO allows TCP to create large segments (up to 64KB), which are segmented by the NIC or kernel AFTER the netfilter hook. It is impossible to authenticate (MAC) segments that do not yet exist.

**Solution - Combined Approach (Option B):**

1. **Primary Mechanism:** On the first KEYED packet, sk->sk_gso_type = 0 is set — TCP stops creating GSO segments for this socket. All packets arrive with the MSS size (1460B) and are encrypted via the normal path.

2. **Fallback (race window):** If a GSO packet was created before setting `sk_gso_type=0`:
- `skb_gso_segment(skb, 0)` — software segmentation into MSS sizes
- Each segment is linearized, encrypted with ChaCha20, and signed with a Poly1305 MAC (TM option)
- Segments are reinjected via `ip_local_out()` with `skb->mark = TCPS_SKB_MARK` to prevent loops
- The original GSO skb is deallocated (`NF_STOLEN`)

**Performance:** Disabling GSO increases the number of packets in the stack (each 64KB GSO → ~44 MSS packets). Tests show:
- curl 50MB: ~200MB/s
- scp 200MB: ~183MB/s
- At rates >500Mbps, the throughput reduction may be noticeable.

**Check:** In dmesg, no `GSO segmenting` = the primary mechanism is active (GSO is disabled before packet creation).
...
**Behavior with a strip option in the middlebox:**
...
## Both sides have a modulus (PSK exchange)
...
| Stage | SSH (port 22) | Othere TCP |
|------|--------------|------------|
| Module loaded | OK (skip) | Encrypted (PSK + MAC) |
| `rmmod` | OK (without module) | Existing keys are broken |
| `insmod` (before discovery) | OK | New keys are weak (no peer) |
| After DISCOVER + KEYXCHG | OK | New keys are DH fallback (before verify) |
| After verify | OK | New keys are full PSK |
...
- **DH fallback PSK** — `KDF = ChaCha20_KDF(DH_shared, "TCPS-FB")` — a separate position, domain-separated
...
Logic and Vulnerabilities. Fixes by category:
...
| H-3 | GSO packets are encrypted without MAC (bit-flipping) | GSO is disabled on the socket (`sk_gso_type=0`); fallback — `skb_gso_segment()` + encryption of each segment |
...
| M-6 | `kernel_sendmsg` inside `rcu_read_lock` during rotationai | Collecting addresses under RCU, sending outside of RCU |
...
| M-11 | KEYXCHG_AUTH FAILED → REJECT blocks PSK during a race condition | WARN + continue as plain KEYXCHG (MITM is impossible without a private key) |

## LOW/INFO (retained)

| Vulnerability | Cause |
|-----------|---------|
| MAC is 4 bytes instead of 16 | TM option 12B won't fit in most packets |
...
| First exchange is vulnerable to MITM | Detectable via fingerprint verify |
...
| Timing attack defense | `tcps_ct_memcmp` for MAC and KEYXCHG_AUTH |
| Downgrade protection | Probe option - absence → fallback to plain TCP |
...
| GSO handled | GSO is disabled on the socket (`sk_gso_type=0`), fallback segmentation + encryption |
...
| Options may not fit | With SACK blocks + TM (8B), TCP option space may exceed 40B |
| ACK-only without MAC | Packets without payload are not signed |
| RST in cleartext | Partial protection: RST without MAC with peer_has_mac → DROP |
